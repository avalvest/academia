...
function($, window) {
    ...
    customizer.listenForEvents = function() {
        ...
        if (anchoPantalla > 992) {
            // oculta el indice para tamaños medianos y grandes
            $('.oculta-indice').on('click', function(e) {

                if ($('#app-main .container .col-md-2.indice').width() == 53) {
                    $('#app-main .container .col-md-10.player').css('width', '');
                    $('#app-main .container .col-md-2.indice').css('width', '');
                    $('.despliega').show();
                      $('#irApag span ').show();

                } else {
                    var ancho = $('#app-main .container .app-content > .row').width();
                    $('#app-main .container .col-md-10.player').css('width', '93%');
                    $('#app-main .container .col-md-2.indice').css('width', '65px');
                    $('.despliega').hide();
                    $('#categories-list').scrollTop(0);
                      $('#irApag span ').hide();
                    setTimeout(function() {
                        $('#categories-list').scrollTop($('.activo').position().top - centroIframe);
                    }, 500);
                }
                $('.app-content').toggleClass("no-indice");
            });

        } else if (anchoPantalla <= 992) {
            // oculta el indice para tamaños pequeños
            altoMenuNavLat();

            $('.oculta-indice').on('click', function(e) {
                  $('.app-actions-list').css('overflow-y', 'hidden');
                altoMenuNavLat();
                $('.app-action-panel').css('left', '-600px');
                $('#botonSecundario').css('display', 'block');

                setTimeout(function() {
                    $('.app-content').toggleClass("no-indice");
                }, 100);
            });
            $('.oculta-indice-secundario').on('click', function(e) {
                $('.app-actions-list').css('overflow-y', 'scroll');
                altoMenuNavLat();
                $('.app-action-panel').css('left', '0px');

                setTimeout(function() {
                    $('#botonSecundario').css('display', 'none');
                }, 100);
                $('.app-content').toggleClass("no-indice");
            });
        }
...
function altoMenuNavLat() {
    var altoContenedor = $('#contenedorObject').height();
    $('.app-action-panel').css({
        height: altoContenedor
    });
}

// El siguiente condicional evita que aparezca como fondo la imagen de "cargando contenido" en la página 000

if ($('#navegador').attr('src').slice(0, -5).substr(-3) === 000) {
    $('#contenedorObject').removeClass('fondoContenedorObject');
} else {
    $('#contenedorObject').addClass('fondoContenedorObject');
}
